...
class KTDataset(Dataset):
    ...
    def load_dataset(self):
        ...
        if self.mode == 1: # test mode
            ...
            df = df.merge(df_pc, on = ['user_id', 'problem_id', 'concept_id', 'correct'], how = 'outer') # combine with pc data
            ...
            test_problem_num = int(df_test['problem'].max() + 1)
            test_concept_num = int(df_test['concept'].max() + 1)
            ...
            print('Number of problems: {}'.format(test_problem_num))
            print('Number of concepts: {}\n'.format(test_concept_num))
        ...
        concept_num = int(df['concept'].max() + 1)
        print('Number of problems: ', question_num)
        print('Number of concept: ', concept_num)
        self.hidden_dim = 128
        if self.mode == 0:
...
def pad_collate_akt(batch):
    ...
    if len(features_problem[0]) < 496:
        features_problem[0].extend([0] * (496-len(features_problem[0])))
        features_concept[0].extend([0] * (496-len(features_concept[0])))
        questions_problem[0].extend([0] * (496-len(questions_problem[0])))
        questions_concept[0].extend([0] * (496-len(questions_concept[0])))
        answers[0].extend([-1] * (496-len(answers[0])))
    ...
    return feature_problem_pad.to(device), feature_concept_pad.to(device), question_problem_pad.to(device), question_concept_pad.to(device), answer_pad.to(device)
